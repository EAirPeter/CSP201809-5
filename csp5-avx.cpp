...
inline void NttFwd(U32 A[], int Exp) {
    ...
    default: {
        ...
        __m256i vm64 = _mm256_set1_epi64x(Mod);
        __m256i vbmm = _mm256_set1_epi64x(BmM);
        ...
        for (int i = Exp - 1; i >= 3; --i) {
            ...
            for (U32 j = 0; j < Len; j += 2u << i) {
                ...
                for (U32 k = 0; k < ChkSiz; k += 8) {
                    ...
                    __m256i vta = _mm256_add_epi32(va, vb);
                    __m256i vtb = _mm256_sub_epi32(vta, _mm256_andnot_si256(_mm256_cmpgt_epi32(vm32, vta), vm32));
                    _mm256_store_si256((__m256i*) (A_ + k), vtb);
                    __m256i vtc = _mm256_sub_epi32(va, vb);
                    __m256i vc = _mm256_add_epi32(vtc, _mm256_and_si256(_mm256_cmpgt_epi32(vb, va), vm32));
                    __m256i vw0 = _mm256_shuffle_epi8(vw, vm0);
                    __m256i vw1 = _mm256_shuffle_epi8(vw, vm1);
                    {
                        __m256i vmul0 = _mm256_mul_epi32(_mm256_shuffle_epi8(vc, vm0), vw0);
                        __m256i vmul1 = _mm256_mul_epi32(_mm256_shuffle_epi8(vc, vm1), vw1);
                        ...
                        _mm256_store_si256((__m256i*) (B_ + k), vtd);
                    }
                    {
        ...
        for (int i = 3; i < Exp; ++i) {
            ...
            for (U32 j = 0; j < Len; j += 2u << i) {
                ...
                for (U32 k = 0; k < ChkSiz; k += 8) {
                    ...
                    __m256i vw1 = _mm256_shuffle_epi8(vw, vm1);
                    {
                        __m256i vmul0 = _mm256_mul_epi32(_mm256_shuffle_epi8(vb, vm0), vw0);
                        __m256i vmul1 = _mm256_mul_epi32(_mm256_shuffle_epi8(vb, vm1), vw1);
                        __m256i vlow = _mm256_blend_epi32(vmul0, _mm256_shuffle_epi32(vmul1, 0xb1), 0xaa);
                        __m256i vquo0 = _mm256_srli_epi64(_mm256_mul_epi32(_mm256_srli_epi64(vmul0, 29), vbmm), BmW);
                        __m256i vquo1 = _mm256_srli_epi64(_mm256_mul_epi32(_mm256_srli_epi64(vmul1, 29), vbmm), BmW);
                        __m256i vval0 = _mm256_mul_epi32(vquo0, vm64);
                        ...
                        __m256i vtb = _mm256_add_epi32(vta, _mm256_and_si256(_mm256_cmpgt_epi32(vzero, vta), vm32));
                        __m256i vtc = _mm256_add_epi32(vtb, _mm256_and_si256(_mm256_cmpgt_epi32(vzero, vtb), vm32));
                        vb = _mm256_sub_epi32(vtc, _mm256_andnot_si256(_mm256_cmpgt_epi32(vm32, vtc), vm32));
                    }
                    {
                        __m256i vmul0 = _mm256_mul_epi32(vw0, vwn);
                        __m256i vmul1 = _mm256_mul_epi32(vw1, vwn);
                    ...
                    _mm256_store_si256((__m256i*) (A_ + k), vtb);
                    __m256i vtc = _mm256_sub_epi32(va, vb);
                    __m256i vtd = _mm256_add_epi32(vtc, _mm256_and_si256(_mm256_cmpgt_epi32(vb, va), vm32));
                    _mm256_store_si256((__m256i*) (B_ + k), vtd);
                }
            }
        }
