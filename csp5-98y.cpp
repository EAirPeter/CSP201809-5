...
inline void NttImpl(U32 A[], int Exp, U32 Wb) {
    U32 Len = 1u << Exp;
    U32 Ws[MaxExp];
    Ws[0] = Wb;
    for (int i = 1; i < Exp; ++i)
    ...
    for (int i = 0; i < Exp; ++i) {
        ...
        for (U32 j = 0; j < Len; j += 2u << i) {
            ...
            for (U32 k = j; k < j + ChkSiz; ++k) {
                U32 t = MMul(W, A[k + ChkSiz]);
                A[k + ChkSiz] = MSub(A[k], t);
                A[k] = MAdd(A[k], t);
                W = MMul(W, Wn);
            }
        }
...
inline void NttFwd(U32 A[], int Exp) {
    NttImpl(A, Exp, WbFwd[Exp]);
}

inline void NttInv(U32 A[], int Exp) {
    NttImpl(A, Exp, WbInv[Exp]);
    U32 Len = 1u << Exp;
    for (U32 i = 0; i < Len; ++i)
...
inline void Rem(U32 A[]) {
    reverse_copy(A + M, A + M + M, TC);
    NttFwd(TC, Exp);
    for (U32 i = 0; i < Len; ++i)
        TC[i] = MMul(TC[i], FRInvNtt[i]);
    NttInv(TC, Exp);
    reverse(TC, TC + M);
    fill(TC + M, TC + M + M - 1, 0);
    NttFwd(TC, Exp);
    for (U32 i = 0; i < Len; ++i)
