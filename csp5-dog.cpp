...
inline void NttImpl(U32 A[], int Exp, U32 Base) {
    U32 Len = 1u << Exp;
    ...
    for (int i = 0; i < Exp; ++i) {
        ...
        for (U32 j = 0; j < Len; j += 2u << i) {
            ...
            for (U32 k = j; k < j + ChkSiz; ++k) {
                U64 even = A[k];
                U64 odd = (U64) A[k + ChkSiz] * W;
                U32 left = (U32) ((even + odd) % Mod);
                U32 right = (U32) ((even + (U64) Mod * Mod - odd) % Mod);
                A[k] = left;
                A[k + ChkSiz] = right;
                W = MMul(W, Wn);
            }
...
inline void NttFwd(U32 A[], int Exp) {
    NttImpl(A, Exp, MPow(G, (Mod - 1) >> Exp));
}

inline void NttInv(U32 A[], int Exp) {
    NttImpl(A, Exp, MPow(GInv, (Mod - 1) >> Exp));
    U32 Len = 1u << Exp;
    U32 LenInv = MInv(Len);
    for (U32 i = 0; i < Len; ++i)
        A[i] = MMul(A[i], LenInv);
...
int main() {
    ...
    freopen(R"???(D:\code\algo\std14\o.txt)???", "w", stdout);
#endif
    FetchAll();
    M = ReadU<U32>();
    U64 L = ReadU<U64>();
